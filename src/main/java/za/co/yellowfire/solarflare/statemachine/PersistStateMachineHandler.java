package za.co.yellowfire.solarflare.statemachine;
 ...
 *
 * Revision: Changed to handle generics
...
public class PersistStateMachineHandler<S,E> extends LifecycleObjectSupport {
	 ...
	 */
	public void handleEventWithState(Message<E> event, S state) {
		stateMachine.stop();
	...
	private class CompositePersistStateChangeListener 
				extends AbstractCompositeListener<PersistStateChangeListener<S, E>> 
				implements PersistStateChangeListener<S, E> {
		...
		public void onPersist(
			...
			for (Iterator<PersistStateChangeListener<S, E>> iterator = getListeners().reverse(); iterator.hasNext();) {
				iterator.next().onPersist(state, message, transition, stateMachine);
			}
		}
	}
}
